...
test("ArrayList as reverse stack", () => {
  const start = performance.now();
  const list = new ArrayList<number>();
...
test("ArrayList insert", () => {
  ...
  expect(list.length).toBe(0);
  list.unshift("foo", "baz");
  list.splice(0);
  list.splice(0, 0, "baa");
  list.splice(1, 0, "baz");
  expect(() => list.splice(5, 0, "boo")).toThrow();
  expect([...list]).toEqual(["baa", "baz", "foo", "baz"]);
});

test("ArrayList insert shift if sublist full", () => {
  const list = new ArrayList<string>();
  expect(list.length).toBe(0);
  list.splice(0, 0, "foo");
  for (let i = 0; i < 201; i++) {
...
test("ArrayList remove will autoremove sublist", () => {
  const list = new ArrayList<string>();
  expect(list.length).toBe(0);
  list.splice(0, 0, "foo");
  list.splice(1, 0, "baz");
